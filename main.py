...
def scrapReview(comment):
    ...
    productRate = comment.find(
        "span", class_="user-post__score-count").text
    commentContent = comment.find("div", class_="user-post__text").text
    recommendation = comment.find(
        "span", class_="user-post__author-recomendation").text
    return Review.ReviewComment(reviewId, authorName, productRate, commentContent, recommendation)
